...
export const PLAN_COMPTABLE: Record<string, string> = {
  ...
  "759": "REPRISES DE CHARGES POUR DEPRECIATIONS ET PROVISIONS POUR RISKS A COURT TERME D'EXPLOITATION",
  ...
  "779": "REPRISES DE CHARGES POUR DEPRECIATIONS ET PROVISIONS POUR RISKS A COURT TERME FINANCIERES",
  ...
  "849": "REPRISES DE CHARGES POUR DEPRECIATIONS ET PROVISIONS POUR RISKS A COURT TERME H.A.O.",
  ...
  "88": "RESULTAT DES CAISSES D'ASSURANCE MALADIE",
  "881": "ETAT",
...
export const NATURE_MAP: Record<string, string> = {
  '1': 'Passif',
  '2': 'Actif',
  '3': 'Actif',
  '4': 'Mixte',
...
export const JOURNAL_NAMES: Record<string, string> = {
  'AC': 'Achats',
  'VE': 'Ventes',
  'BQ': 'Banque',
  'CA': 'Caisse',
...
export const JOURNAL_ICONS: Record<string, string> = {
  'AC': '🛒',
  'VE': '💰',
  'BQ': '🏦',
  'CA': '💵',
...
export function searchComptes(query: string, classe?: string): Array<{ code: string; libelle: string }> {
  const q = query.toLowerCase().trim();
  if (!q && !classe) return [];
  
  return Object.entries(PLAN_COMPTABLE)
    .filter(([code, lib]) => {
      if (classe && !code.startsWith(classe)) return false;
      if (!q) return true;
      return code.startsWith(q) || lib.toLowerCase().includes(q);
    })
    .slice(0, 15)
    .map(([code, libelle]) => ({ code, libelle }));
}

export function getLibelleCompte(code: string): string {
  return PLAN_COMPTABLE[code] || '';
}

export function getClasse(code: string): string {
  return code[0] || '';
}
