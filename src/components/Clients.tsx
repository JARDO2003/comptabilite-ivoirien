...
export default function ClientsView() {
  ...
  const openEdit = (c: Client) => {
    setEditing(c);
    setNom(c.nom); setTel(c.telephone); setEmail(c.email);
    setAdresse(c.adresse); setVille(c.ville); setNif(c.nif);
    setShowModal(true);
  };
  ...
  const handleSave = () => {
    ...
    const client: Client = {
      id: editing?.id || genId(),
      code: editing?.code || `CLI-${String(clients.length + 1).padStart(3, '0')}`,
      nom, telephone: tel, email, adresse, ville, nif, notes: '',
      caTotal: editing?.caTotal || 0, soldeDu: editing?.soldeDu || 0
    };
    if (editing) updateClient(client); else addClient(client);
    setShowModal(false);
  };
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">Répertoire et suivi des clients</p>
        </div>
      ...
      <div className="overflow-x-auto rounded-md border border-gray-200 shadow-sm">
        <table className="w-full text-xs">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={6} className="text-center py-8 text-gray-400 text-sm">Aucun client</td></tr>
            ) : filtered.map(c => (
              <tr key={c.id} className="border-b border-gray-100 hover:bg-gray-50">
                <td className="px-3 py-2 font-mono text-amber-600">{c.code}</td>
                <td className="px-3 py-2 text-gray-900 font-medium">{c.nom}</td>
                <td className="px-3 py-2 text-gray-600">{c.telephone || '—'}</td>
                <td className="px-3 py-2 text-gray-600">{c.email || '—'}</td>
                <td className="px-3 py-2 text-right font-mono text-gray-900">{c.soldeDu.toLocaleString('fr-FR')} FCFA</td>
                <td className="px-3 py-2 text-center">
                  <div className="flex items-center justify-center gap-1">
                    <button onClick={() => openEdit(c)} className="text-gray-400 hover:text-amber-600 p-1 transition-all">
                      <Pencil size={14} />
                    </button>
                    <button onClick={() => deleteClient(c.id)} className="text-gray-400 hover:text-red-600 p-1 transition-all">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(10,11,16,0.7)' }}>
          <div className="bg-white rounded-xl w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between p-4 border-b border-gray-200">
              <h2 className="font-serif text-lg font-bold text-gray-900">{editing ? 'Modifier' : 'Nouveau'} client</h2>
