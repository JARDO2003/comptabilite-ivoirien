...
// Les clés API Mistral et l'ordre des modèles sont gérés via server.html
...
let mistralKeyIdx     = 0;    // Index rotation clés
...
async function loadServerConfig() {
  try {
    ...
    if (keysSnap.exists()) {
      ...
      MISTRAL_API_KEYS = rawKeys.map(k => k.value).filter(Boolean);
    }

    if (modelsSnap.exists()) {
      MISTRAL_MODELS = modelsSnap.data().list || [];
    }
    ...
    aiServiceAvailable = MISTRAL_API_KEYS.length > 0;
    updateServiceAvailabilityUI();
    console.log(`[COMEO] Config chargée — ${MISTRAL_API_KEYS.length} clé(s) Mistral, ${MISTRAL_MODELS.length} modèle(s)`);
  } catch (e) {
...
function updateServiceAvailabilityUI() {
  ...
  if (!banner) return;
  const show = !aiServiceAvailable || MISTRAL_API_KEYS.length === 0;
...
function isAiServiceReady() {
  return aiServiceAvailable && MISTRAL_API_KEYS.length > 0;
}
...
9. ACHAT MARCHANDISES = minimum 2 écritures (AC + BQ), 3 si stock suivi (AC + IN + BQ)
10. VENTE MARCHANDISES = minimum 2 écritures (VE + BQ), 3 si stock suivi (VE + IN + BQ)
11. JAMAIS une seule écriture pour achat ou vente — c'est toujours incomplet et faux
12. Si l'utilisateur demande POURQUOI ce nombre d'écritures : expliquer clairement la logique SYSCOHADA (séparation des flux : flux commercial / flux stock / flux trésorerie)
...
try {
    ...
    // ══ APPEL CLAUDE — moteur principal (raisonnement expert) ══
    const claudeData = await callClaude(conversationHistory, systemPrompt);
    if (claudeData) {
      data = claudeData;
      console.log('[COMEO] Réponse Claude Sonnet OK');
    }

    // ══ FALLBACK MISTRAL si Claude indisponible ══
    if (!data && MISTRAL_API_KEYS.length > 0) {
      console.warn('[COMEO] Claude indisponible → fallback Mistral');
    ...
    appendMsg(context, 'ai', "⏳ Erreur de connexion au service IA. Veuillez réessayer.");
  }
  isAILoading = false;
...
// STT (Web Speech API) → Mistral LLM → TTS (Web Speech API)
// ══════════════════════════════════════════════════════════
...
// ── Envoi à Mistral avec contexte comptable ──
...
- Fournisseurs (401) : ${fnPDF((map['401']?.credit||0)-(map['401']?.debit||0))} FCFA à payer`;
  ...
  try {
    const data = await callMistral(robotConvHistory, systemRobot);
    if (!data) throw new Error('Erreur API Mistral');
    ...
    robotSpeak("Désolé, je n'ai pas pu me connecter à Mistral. Veuillez réessayer dans un instant.", { skipBubble: true });
    setRobotBubble('<span class="service-msg-inline">⏳ ' + escapeHtml(getAiUnavailableMessage()) + '</span>');
  } finally {
...
// ══════════════════════════════════════════
// APPEL CLAUDE (Anthropic) — Moteur IA principal
// Raisonnement expert garanti — format compatible Mistral
// ══════════════════════════════════════════
async function callClaude(messages, systemPrompt) {
  try {
    ...
    if (!response.ok) {
      console.warn('[COMEO] Claude HTTP ' + response.status);
      return null;
    }
    const data = await response.json();
    const text = data.content?.filter(b => b.type === 'text').map(b => b.text).join('') || '';
    console.log('[COMEO] Claude OK — in:' + data.usage?.input_tokens + ' out:' + data.usage?.output_tokens);
    return { choices: [{ message: { content: text } }], _provider: 'claude' };
  } catch (e) {
    console.warn('[COMEO] Claude réseau:', e.message);
    return null;
  }
}
